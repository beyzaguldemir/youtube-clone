...
const Comments = ({data}) => {
  ...
  return (
    <div className='my-6'>
      ...
      <input type="text" className='w-full bg-transparent outline-none mb-5 p-2 border-b' placeholder='Yorum Ekleyiniz...' />
      {data?.data.map((i,index)=>(
        <div key={index} className='flex gap-2 items-center px-1 py-4'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            {i?.replyCount>0 && <div className=' w-fit rounded cursor-pointer flex items-center p-1 gap-2 text-blue-500 hover:bg-[#11263D]'><TiArrowSortedDown /> {i?.replyCount} Yanıt </div>}
          </div>
        </div>
      ))}
    </div>
